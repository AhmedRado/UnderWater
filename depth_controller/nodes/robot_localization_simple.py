...
statesX_pub_simple = rospy.Publisher("KalmanX_simple", Float64, queue_size=1)
statesY_pub = rospy.Publisher("KalmanY_simple", Float64, queue_size=1)
statesZ_pub = rospy.Publisher("KalmanZ_simple", Float64, queue_size=1)


rmseX_pub_simple = rospy.Publisher("rmseX_simple", Float64, queue_size=1)
rmseY_pub_simple = rospy.Publisher("rmseY_simple", Float64, queue_size=1)
rmseZ_pub_simple = rospy.Publisher("rmseZ_simple", Float64, queue_size=1)
...
Kalman_msgX_simple = Float64()
Kalman_msgY_simple = Float64()
Kalman_msgZ_simple = Float64()

rmseX_simple  = Float64()
rmseY_simple  = Float64()
rmseZ_simple  = Float64()
...
def Kalman_function():
    ...
    Kalman_msgX_simple .data = X[0,0]


    Kalman_msgY_simple .data = X[1,0]


    Kalman_msgZ_simple .data = X[2,0]


    statesX_pub_simple .publish(Kalman_msgX_simple)
    statesY_pub.publish(Kalman_msgY_simple)
    statesZ_pub.publish(Kalman_msgZ_simple) 
...
if __name__ == "__main__":
    ...
    while not rospy.is_shutdown():
        ...
        if(count<1000):
            mean_x[count] = (Kalman_msgX_simple .data - true_x)**2
            mean_y[count] = (Kalman_msgY_simple .data - true_y)**2
            mean_z[count] = (Kalman_msgZ_simple .data - true_z)**2
            count = count +1
        else:
            count =0
        ...
        rmseX_simple.data = math.sqrt(np.sum(mean_x)/1000)
        rmseY_simple.data = math.sqrt(np.sum(mean_y)/1000)
        rmseZ_simple.data = math.sqrt(np.sum(mean_z)/1000)
        ...
        print('Z')
        print(rmseZ_simple)
        rmseX_pub_simple.publish(rmseX_simple)
        rmseY_pub_simple.publish(rmseY_simple )
        rmseZ_pub_simple.publish(rmseZ_simple )
        rate.sleep()
